...
tags: [splunk,ioc,threat intelligence,framework]
description: test
---
...
```bash
*/5 * * * * awk -F'"' 'NR>1 {gsub(/"/, "", $2); gsub(/"/, "", $4); if ($2) {print $2} else if ($4) {print $4}}' /INSTALL_PATH/splunk/etc/apps/{APP_NAME}/lookups/soc_ioc_intel.csv  > /INSTALL_PATH/splunk/etc/system/static/soc_ioc_intel.csv

```

Bu işlemden sonra aşağıdaki url de bulunan listeyi firewall tarafına Threat Feed olarak ekleyebiliriz.

`https://{SPLUNK_IP}:8089/static/soc_ioc_intel.csv`
